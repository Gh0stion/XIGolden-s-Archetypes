...
function s.initial_effect(c)
	...
	-- 4. Cannot declare attacks
	local e4=Effect.CreateEffect(c)
	e4:SetType(EFFECT_TYPE_SINGLE)
	e4:SetCode(EFFECT_CANNOT_ATTACK)
	c:RegisterEffect(e4)

	-- 5. Add 1 "Starforce" Continuous Spell on Special Summon
	...
	-- 6. Discard to Special Summon Level 4 from Deck (HOPT)
	...
	-- 7. Shuffle from hand to draw (HOPT)
...
s.listed_names={89901622,38615480}

-- Restriction Logic
function s.nosumcon(e)
	local tp=e:GetHandlerPlayer()
	return Duel.IsExistingMatchingCard(s.synfilter,tp,LOCATION_MZONE,0,1,nil)
		or Duel.IsExistingMatchingCard(Card.IsCode,tp,LOCATION_MZONE,0,1,nil,89901622,38615480)
end
...
function s.thfilter(c)
	return c:IsSetCard(0x99b) and c:IsType(TYPE_CONTINUOUS) and c:IsType(TYPE_SPELL) and c:IsAbleToHand()
end
