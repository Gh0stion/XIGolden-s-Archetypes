...
function s.initial_effect(c)
    ...
    local e0c=e0:Clone()
    e0c:SetCode(EFFECT_CANNOT_MSET) -- Cannot be Set
    ...
    -- 4. Search Field Spell on Summon
...
s.listed_names={30411028,38615480}

-- Updated Restriction Logic (Synchro OR Satellites)
function s.nosumcon(e)
    local tp=e:GetHandlerPlayer()
    return Duel.IsExistingMatchingCard(s.synfilter,tp,LOCATION_MZONE,0,1,nil)
        or Duel.IsExistingMatchingCard(Card.IsCode,tp,LOCATION_MZONE,0,1,nil,30411028,38615480)
end
...
function s.spcon(e,c)
    ...
    -- Check the restriction inside the SP PROC condition as well
